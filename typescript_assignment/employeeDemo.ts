...
let employees=new Array<{EmpNo:number,EmpName:string,Department:string,Salary:number}>();
...
function getEmployeeNameByDepartment(department:string){
...
var sortedList=employees.sort((a,b)=>a.EmpName.localeCompare(b.EmpName));
...
var reversedRecords=employees.reverse();
  ...
  var departmentWiseEmployees = employees.reduce(function(r, o){
    ...
    if (r[key] || (r[key]=[])) r[key].push({Department:key, EmpNo:o.EmpNo,EmpName:o.EmpName,Salary:o.Salary});
    return r;
}, {});

console.log("Department wise records");
...
let numbers1=[1,3,4,5,6,4,5,6];

let sum1= numbers1.reduce((final,current)=>{
